...
function createMockResponseContent(userText) {
  ...
  // New starter bundle dataset queries
  if (lower.includes("sea level") || lower.includes("sea_level") || lower.includes("sea level anomaly")) {
    return "The Sea Level Anomalies layer shows gridded sea level rise data sourced from the Pacific Data Hub SDMX API. The data is fetched dynamically, joined to regional boundaries, and binned into H3 hexagons (Resolution 4 for larger islands, Resolution 5 for small atolls like Tuvalu, Nauru, and Kiribati). Enable the 'Sea Level Rise (H3)' layer in the map panel to view it. Note: If the Pacific Data Hub API is unreachable and no cached data exists, the layer will show as 'unavailable' and I will not make quantitative claims about sea level values.";
  }

  if (lower.includes("power gen") || lower.includes("power generation") || lower.includes("power_gen") || lower.includes("power asset")) {
    return "The Power Generation layer shows annual electricity generation in gigawatt-hours (GWh) per Pacific Island country, sourced from the Pacific Data Hub SDMX API (DF_POWER_GEN). It is a regional choropleth: values are summed across all energy sources and grid-connection types per country for the latest year, and shaded on an orange scale. Enable the 'Power Gen (GWh)' layer in the map panel to view the choropleth. If the Pacific Data Hub API is unreachable and no cached data exists, the layer will show as 'unavailable' and I will not make quantitative claims about power generation totals.";
  }

  if (lower.includes("water access") || lower.includes("drinking water") || lower.includes("safe water") || lower.includes("water_access") || lower.includes("sh_h2o_safe")) {
    return "The Safely Managed Drinking Water Access layer shows the proportion of population with access to safe drinking water by region/atoll, sourced from the Pacific Data Hub SDMX API (SDG indicator SH_H2O_SAFE). Enable the 'Water Access' layer in the map panel to view the choropleth. If the Pacific Data Hub API is unreachable and no cached data exists, the layer will show as 'unavailable' and I will not make quantitative claims about water access percentages.";
  }
