...
| :--- | :--- |
| **Vite's dev server serves an HTML file that is not in the build input.** Decision 1 now depends on this specific behaviour: production lists `index.html` only, and `/workbench.html` is expected to be reachable in dev anyway. The API half is settled (see Verified facts); this half is not, and was not run | **Verified 2026-07-31 by task 2.2** — `npm run dev` serves `/workbench.html` (HTTP 200, the workbench HTML, module transforms clean) with `build.rolldownOptions.input` naming `index.html` only, and `npm run build` emits no workbench artefact. No fallback Vite config was needed |
| A bundle scan can reliably detect a fixture module in production output. **Oxc minification** (now confirmed as the default) may leave no recognisable path or identifier, in which case a path-based guard passes vacuously — the worst possible failure for this change, because it fails in the reassuring direction | **Verified 2026-07-31 by task 1.1a** — the guard is keyed on a sentinel string literal, and the negative control (deliberately importing a fixture from the application entry and building) was **seen to fail** with `fixture sentinel "fixture-data-workbench-sentinel-v1" found in the production bundle: dist/assets/index-*.js`. Literals survive Oxc; a path-based scan would not have. |
| **5–8 categorical classes are simultaneously legible** — the actual research question | **Answered 2026-07-31 (tasks.md 5.3)** — measured at 3/5/8/16 over real tikina geometry: the threshold sits between 8 and 16 classes, and it is a spatial-extent failure (small-island classes render as slivers at 16), not a colour failure (all adjacent ΔE00 ≥ 13.3 even under deuteranopia). See `docs/viz-component-workbench-verification.md`. |
